...
public class TurnTaskCmd extends NeedsActiveTaskCmd<Void> {

    protected String businessCode;
    protected String targetUserId;
    protected String comment;

    public TurnTaskCmd(String taskId, String targetUserId, String comment) {
        this(null,taskId,targetUserId,comment);
    }

    public TurnTaskCmd(String businessCode, String taskId, String targetUserId, String comment) {
        super(taskId);
    ...
    protected Void execute(CommandContext commandContext, TaskEntity task) {
        ...
        if (!StringUtils.equals(Authentication.getAuthenticatedUserId(), task.getAssignee())) {
            throw new FlowableException("非任务办理人不能转办");
        }

        User user =  CommandContextUtils.findUser(commandContext, Authentication.getAuthenticatedUserId());

        User targetUser = CommandContextUtils.findUser(commandContext, targetUserId);
        String turnTaskComment = String.format("%s 转办任务给 %s，原因：%s", user.getDisplayName(), targetUser.getDisplayName(), StringUtils.defaultString(comment));
        // 添加评语
        processEngineConfiguration.getTaskService().addComment(taskId,
                task.getProcessInstanceId(),
                turnTaskComment);

        processEngineConfiguration.getTaskService().setAssignee(taskId,targetUserId);
        return null;
    }
}
